...
const handlefilechange =  (e)=>{  // this is called when user uploads a file , it is imp because input will store fill but react state 
  // doesnt know about it .. so we have to call this function otherwise upload file will be null ... 
...
<div className='flex gap-3 pt-2'>
 ...
 <div className='fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm'>
  <div className='relative w-full max-w-md bg-white/95 backdrop-blur-xl border border-slate-200/60 rounded-2xl shadow-2xl shadow-slate-900/20 p-8'>
   {/* Close button */} 
<button onClick={()=>setisdeletemodalopen(false)} className='absolute top-6 right-6 w-8 h-8 flex items-center justify-center rounded-lg text-slate-400 hover:text-slate-600 hover:bg-slate-100 transition-all duration-200'><X className='w-5 h-5' strokeWidth={2} /></button>
{/* Model header*/}
<div className='mb-6'>
  <div className='w-12 h-12 rounded-xl bg-linear-to-r from-red-100 to-red-200 flex items-center justify-center mb-4'><Trash2  className='w-6 h-6 text-red-600' /></div>
  <h2 className='text-xl font-medium text-slate-900 tracking-tight'>Confirm Deletion</h2>
</div>
{/* content*/}
<p className='text-sm text-slate-600 mb-6'>Are you sure you want to delete the document:{" "} <span className='font-semibold text-slate-900'>{selecteddoc?.title}</span>?This action cannot be undone</p>
