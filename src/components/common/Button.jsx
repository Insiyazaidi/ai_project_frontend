import React from 'react'

const Button = ({children , onClick , type="button" , disabled = false , className = "" , variant="primary" , size="md"}) => {
 const basestyle="inline-flex items-center justify-center gap-2 font-semibold rounded-xl transition-all duration-200 active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed disabled:active:scale-100 whitespace-nowrap"
const variantstyle = {
    primary:"bg-linear-to-r from-primary-dark to-primary text-white shadow-lg shadow-soft/25 ",  // eg upload  button ke liye
    secondary:"bg-slate-100 text-slate-700 hover:bg-slate-200",  // eg - delete button ke liye 
    outline:"bg-white border-2 border-slate-200 text-slate-700 hover:bg-slate-50 hover:border-slate-300"
}
 // hr button ke liye ek  nya style bna rhe h 
const sizestyle = {
    sm:"h-9 px-4 text-xs " , 
    md:"h-11 px-5 text-sm" ,
}
// Ye button ke andar ka content hai - children
return (
    <button type={type} onClick={onClick} disabled={disabled} className={[basestyle , variantstyle[variant] , sizestyle[size] , className ].join(" ")}>
{children}  
    </button>
)

}

export default Button