...
<p className='text-sm text-slate-500 mb-8 max-w-sm leading-relaxed '>{description}</p>
{
    buttontext && onactionclick && (
        <button onClick={onactionclick} className='group relative inline-flex items-center gap-2 px-6 h-11 bg-linear-to-r from-primary-dark to-primary hover:from-primary hover:to-primary-dark text-white font-semibold text-sm rounded-xl  transition-all duration-200 active:scale-95 focus:outline-none focus:ring-4 focus:ring-soft overflow-hidden '>
            <span className='relative z-10 flex items-center gap-2'><Plus className='w-4 h-4 ' strokeWidth={2.5}/>{buttontext}</span>
